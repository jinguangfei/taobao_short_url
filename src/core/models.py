...
class BaseModel(models.Model):
    ...
    async def to_dict(self, m2m: bool = False, fk: bool = False, exclude_fields: list[str] | None = None) -> Dict[str, Any]:
        ...
        if fk:
            tasks = [
                self.__fetch_fk_field(field, exclude_fields)
    ...
    async def __fetch_fk_field(self, field, exclude_fields):
        fk_obj = await getattr(self, field)
        if not fk_obj:
            return field, None

        values = await fk_obj.to_dict(exclude_fields=exclude_fields)
        return field, values
